if (Object.const_defined?("Formtastic") && Gem.loaded_specs["formtastic"].version.version[0,1].to_i > 1)
  ...
  class RichPickerInput < ::Formtastic::Inputs::StringInput
    ...
    def preview_image_path
      ...
      if column_type == :integer
        begin
          ...
          @file_format = file.rich_file_content_type

          if @file_format.present? && @file_format.include?("image")
            file.rich_file.url(:rich_thumb) #we ask paperclip directly for the file, so asset paths should not be an issue
          else
            @file_name = file.rich_file.instance.rich_file_file_name
            @file_url = file.rich_file.url
          end
    ...
    def preview
      ...
      if path && (@file_format.present? && @file_format.include?("image"))
        if path == "broken"
          %Q{
            </br></br>
            <div class=preview>
              <img src="broken.jpg" #{klass} #{style} />
            </div>
          }.html_safe
        else
          %Q{
      ...
      elsif path && (@file_format.present? && @file_format.include?("pdf"))
        %Q{
          </br></br>
          <div class=preview>
